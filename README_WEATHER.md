...
│                     Flask REST API                          │
│                                                             │
│  POST /weather/sync    -  Fetch & insert alerts            │
...
```
1. Sync     →  2. Embed    →  3. Search
(Fetch NWS)   (Vectorize)    (Query)
```

### Option A: Using the Flask REST API
...
#### 2. Sync Weather Alerts

```bash
curl -X POST http://localhost:5000/weather/sync \
  -H "Content-Type: application/json" \
  -d '{
    "cities": ["Boston, MA", "Austin, TX", "New York, NY", "Denver, CO", "San Francisco, CA"]
  }'
```
...
Run the Databricks notebook `ingest_weather_alert_report` which orchestrates the entire pipeline:

1. **Cell 1-4**: Configuration and setup
...
```
Weather-Retrieval-Service/
├── app.py                      # Flask REST API (sync + search endpoints)
├── streamlit_app.py            # Streamlit UI (optional, for manual testing)
...
├── app.yaml                    # Databricks Apps config (for Streamlit)
├── README_WEATHER.md           # This file
└── ingest_weather_alert_report # Databricks notebook (alternative to scripts)
```
